...
const ProductTable = () => {
    ...
    products && products.forEach((item) => {
        rows.unshift({
            ...
            brand: item.brand ? (typeof item.brand === 'object' ? item.brand.name : item.brand) : "N/A",
            // FE-008 FIX: backend now populates category — read .name instead of showing raw ObjectId
            category: item.category ? (typeof item.category === 'object' ? item.category.name : item.category) : 'N/A',
            stock: item.stock,
            price: item.price,
    ...
    return (
        <>
            <MetaData title="Admin Products | GlossyVibes" />
