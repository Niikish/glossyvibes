...
const dt = new Date();
const ordertime = [dt.getMonth(), dt.getFullYear(), dt.getFullYear() - 1, dt.getFullYear() - 2];
...
const MyOrders = () => {
        ...
        if (status && orderTime) {
            if (+orderTime === dt.getMonth()) {
                const filteredArr = orders.filter((order) => order.orderStatus === status &&
                    ...
                    new Date(order.createdAt).getFullYear() === dt.getFullYear()
                );
    ...
    return (
        <>
            <MetaData title="My Orders | GlossyVibes" />
                    ...
                    <div className="flex-1">
                        ...
                        {loading ? <Loader /> : (
                            <div className="flex flex-col gap-3 sm:mr-4 overflow-hidden">
                                ...
                                {/* <!-- searchbar --> */}
                                <form onSubmit={searchOrders} className="flex items-center justify-between mx-1 sm:mx-0 lg:w-10/12 bg-white border rounded hover:shadow">
                                    ...
                                    <button type="submit" className="h-full text-sm px-4 py-2.5 text-white bg-primary-blue hover:bg-blue-600 rounded-r flex items-center gap-1 group">
                                ...
                                {orders && filteredOrders.map((order) => {
                                    ...
                                    return (
                                        orderItems.map((item, index) => (
                                            <OrderItem {...item} key={index} orderId={_id} orderStatus={orderStatus} createdAt={createdAt} deliveredAt={deliveredAt} invoiceUrl={order.invoiceUrl} />
                                        ))
                                    )
                                })}
                            </div>
